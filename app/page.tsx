...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="px-6 py-5 grid grid-cols-4 gap-3 border-b border-[#1a1a1a]">
        {[
        ...
        <div className="w-52 shrink-0 space-y-3">
          ...
          <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
            ...
            <input
              ...
              className="w-full bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 text-sm text-white placeholder-gray-700 focus:outline-none focus:border-yellow-400 transition"
          ...
          <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-4">
            ...
            <select
              ...
              className="w-full bg-[#0a0a0a] border border-[#222] rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-yellow-400"
            >
            ...
            <div className="flex gap-2">
              {["All", "Free", "VIP"].map((p) => (
                <button
                  ...
                  className={`flex-1 py-1 rounded text-xs font-bold transition ${pricing === p ? "bg-yellow-400 text-black" : "bg-[#0a0a0a] border border-[#222] text-gray-500 hover:border-yellow-400"}`}
                >{p}</button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {["All", "Scripts", "MLO", "Vehicles", "Clothing", "Maps"].map((c) => (
                <div
                  ...
                  className={`text-sm cursor-pointer px-2 py-1.5 rounded transition ${category === c ? "text-yellow-400 font-bold bg-yellow-400/10 border border-yellow-400/20" : "text-gray-500 hover:text-yellow-400 hover:bg-[#0a0a0a]"}`}
                >{c}</div>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1">
          ...
          {list.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-4">
              {list.map((r: any) => (<ResourceCard key={r.id} r={r} />))}
            </div>
          )}
        </div>
      </div>
